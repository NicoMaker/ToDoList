...
app.put('/api/todos/:id', (req, res) => {
  ...
  db.get('SELECT * FROM todos WHERE id = ?', [id], (err, existing) => {
    ...
    db.run(sql, [newTitle, newDescription, newCompleted, newPriority, id], function (err2) {
      if (err2) return res.status(500).json({ error: err2.message });
...
app.delete('/api/todos/:id', (req, res) => {
  db.run('DELETE FROM todos WHERE id = ?', [req.params.id], function (err) {
    if (err) return res.status(500).json({ error: err.message });
    if (this.changes === 0) return res.status(404).json({ error: 'Todo non trovato' });
    res.json({ message: 'Todo eliminato con successo', id: Number(req.params.id) });
  });
});
